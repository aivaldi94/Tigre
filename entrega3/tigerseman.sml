...
fun transExp((venv, tenv, levNest) : ( venv * tenv * tigertrans.level)) : (tigerabs.exp -> expty) =
  let fun error(s, p) = raise Fail ("Error -- línea "^Int.toString(p)^": "^s^"\n")
    ...
    | trexp(CallExp({func, args}, nl)) =  
      ...
      in
        case t of
          TUnit => if eqList m then {exp=callExp(lab,ext,true,lev,argsExp), ty= t}
                               else error("Tipos erroneos",nl)
            | _ => if eqList m then {exp=callExp(lab,ext,false,lev,argsExp), ty= t}
                               else error("Tipos erroneos",nl) 
      end
    ...
    | trexp(OpExp({left, oper=EqOp, right}, nl)) =
      ...
      in 
        if tiposIguales tyl tyr andalso not (tyl=TNil andalso tyr=TNil) andalso tyl<>TUnit
           then {exp= if tiposIguales tyl TString
    ...
    | trexp(AssignExp({var=SimpleVar s, exp}, nl)) = 
      ...
      in if (tiposIguales texp r) then {exp=assignExp{var =evar ,exp = eexp},ty= TUnit}
                                  else error("Tipos erroneos",nl) end 
    ...
    | trexp(WhileExp({test, body}, nl)) =
      let
        ...
        val (exp, ty) = if tipoReal (#ty ttest) = TInt andalso #ty tbody = TUnit
                           then (whileExp {test=(#exp ttest), body=(#exp tbody), lev=topLevel()}, TUnit)
    ...
    | trexp(ForExp({var, escape, lo, hi, body}, nl)) =  
      let
        ...
        val (exp,ty) = if tipoReal (#ty tlo) = TInt andalso tipoReal (#ty thi) = TInt andalso (#ty tbody) = TUnit
                          then (forExp {lo= #exp tlo,hi= #exp thi,var= #exp evar,body= #exp tbody}, TUnit)
                          else if tipoReal(#ty tlo) <> TInt orelse tipoReal (#ty thi) <> TInt
    ...
    | trexp(ArrayExp({typ, size, init}, nl)) = 
      ...
      in if (tiposIguales tinit r) andalso tsize = TInt
            then {exp = tigertrans.arrayExp({init=einit, size=esize}),ty = TArray (r, r3 )}
            else error ("size no es int o tipo de init incorrecto",nl)
      end 
and trvar(SimpleVar s, nl) = 
    ...
    and trdec (venv, tenv) (VarDec ({name,escape,typ=NONE,init},nl)) = 
        ...
        let
          ...
          val niv = tigertrans.levInt(levNest)
          val v = tigertrans.assignExp{var=tigertrans.varDec (acc),exp=eexp} 
        in if (tiposIguales texp r) then (tabRInserta (name, Var {ty=r, access= acc,nivel= niv}, venv),tenv,[v])
                                    else error ("La inicializacion no tiene el tipo de la variable",nl) end  
        ...
        val listPos = map (fn (a,b) => b) xs        
         ...
         fun newEnvs (([], venv,_) : ((recfun * int) list * venv * int list)) : venv list = []
          ...
          let
            ...
            val pos = hd intList
            val auxiliar2 = map (fn a => (a,pos)) auxiliar
            val auxiliar3 = List.map (fn ((a,b),c) => (a,b,c)) auxiliar2
            val nvenv = insertArgs (auxiliar3, nom, venv)
          in 
             nvenv :: (newEnvs (rns, venv,tl intList))
          end
          (* venvs es la lista de entornos con las variables agregadas para cada función *)
         val venvs : venv list = newEnvs(xs, env1, listPos)
    ...
    | trdec (venv,tenv) (TypeDec ts) = 
      let 
              ...
              val tenv' = if (Splayset.numItems ts' = length ts) then tigertopsort.fijaTipos (List.map (fn (r, n) => r) ts) tenv
                                                                 else error("Tipos con el mismo nombre", #2 (hd ts))
            in (venv, tenv', []) end
    in trexp end
